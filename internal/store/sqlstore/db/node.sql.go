...
const nodeFindMAC = `-- name: NodeFindMAC :one
...
func (q *Queries) NodeFindMAC(ctx context.Context, db DBTX, mac null.String) (NodeView, error) {
	row := db.QueryRowContext(ctx, nodeFindMAC, mac)
	var i NodeView
	err := row.Scan(
		&i.ID,
		&i.Name,
...
const nodeResolveFQDN = `-- name: NodeResolveFQDN :many
select nc.ip
from nic as nc
where lower(nc.fqdn) = ?1
`

func (q *Queries) NodeResolveFQDN(ctx context.Context, db DBTX, fqdn null.String) ([]null.String, error) {
	rows, err := db.QueryContext(ctx, nodeResolveFQDN, fqdn)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []null.String
	for rows.Next() {
		var ip null.String
...
const nodeResolveIP = `-- name: NodeResolveIP :many
select nc.fqdn
from nic as nc
where nc.ip = ?1
`

func (q *Queries) NodeResolveIP(ctx context.Context, db DBTX, ip null.String) ([]null.String, error) {
	rows, err := db.QueryContext(ctx, nodeResolveIP, ip)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
