...
drop index if exists nic_mac_idx;

drop index if exists nic_ip_idx;

drop index if exists nic_fqdn_lower_idx;

drop view if exists node_view;

update nic
   set ip = ip || '/' || prefix_len
 where ip is not null and prefix_len is not null;

alter table nic drop column prefix_len;
