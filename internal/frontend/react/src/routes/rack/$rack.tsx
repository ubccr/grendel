...
import { useHostTagsSuspense } from "@/openapi/queries/suspense";
import { Link } from "@tanstack/react-router";
import { Checkbox } from "@/components/ui/checkbox";
import { HostTagsResponse } from "@/openapi/requests";
import {
    DropdownMenu,
...
type rackArr = {
    u: string;
    hosts: HostTagsResponse;
};

function RackTable() {
    const { rack } = Route.useParams();
    const { data, isSuccess } = useHostTagsSuspense({ path: { tags: rack } });
    const [view, setView] = useState(["provision", "tags"]);
                ...
                <TableHeader className="*:text-center">
                    <TableRow className="*:my-auto *:text-center">
                        ...
                        <TableHead className="*:my-auto *:text-center grid grid-cols-3">
                            ...
                            <span>Node</span>
                            <div>
                                <Actions checked={checked.join(",")} length={checked.length} />
                            </div>
                        </TableHead>
                            ...
                            <TableCell className={`grid grid-cols-${u.hosts.length} gap-4`}>
                                {u.hosts.map((host, i) => (
                                    <div key={i} className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                                        ...
                                        <Link to={`/nodes/$node`} params={{ node: host.name }} className="my-auto">
                                            {host.name}
                                        </Link>
                                        <div className="flex justify-center gap-6 sm:justify-end">
                                            {!!view.find((col) => col === "tags") && <TagsList tags={host.tags} />}
                                                    ...
                                                    <Tooltip>
                                                        ...
                                                        <TooltipContent>
                                                            <span>
                                                                https://
                                                                {
                                                                    host.interfaces?.filter((v) => v.bmc == true)?.[0]
                                                                        ?.fqdn
                                                                }
                                                            </span>
                                                        </TooltipContent>
