...
var (
	...
	jobListOptions *cmd.TableOptions
	jobListColumns = []cmd.Column{
		{Name: "Node"},
		{Name: "Job Name"},
		{Name: "State"},
		{Name: "Progress"},
	...
	jobListCmd = &cobra.Command{
		...
		RunE: func(command *cobra.Command, args []string) error {
			...
			nodes, err := cmd.API.GETV1BmcJobs(command.Context(), params)
			if err != nil {
				return cmd.NewApiError(err)
			}

			t := cmd.NewTable(jobListColumns, jobListFilter).Options(*jobListOptions).Empty("-")
...
func init() {
	jobListOptions = cmd.RegisterTableFlags(jobListCmd)
	jobListCmd.Flags().StringSliceVar(&jobListFilter, "filter", nil, "filter table columns by name")
	jobListCmd.RegisterFlagCompletionFunc("filter", cmd.ColumnCompletion(jobListColumns))
