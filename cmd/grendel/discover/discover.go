...
func init() {
	...
	discoverCmd.PersistentPreRunE = func(command *cobra.Command, args []string) error {
		...
		// This hook shadows the root PersistentPreRunE. No api client is built here, lldp was the only discover subcommand that called the server and it lives in grendelctl now.
	...
	shared.Root.AddCommand(discoverCmd)
}
