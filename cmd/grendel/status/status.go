...
var (
	...
	statusCmd = &cobra.Command{
		...
		RunE: func(command *cobra.Command, args []string) error {
			...
			imageList, err := apiclient.API.GETV1Images(command.Context(), params)
			if err != nil {
			...
			hostList, err := apiclient.API.GETV1NodesFind(command.Context(), req)
			if err != nil {
				return apiclient.NewApiError(err)
			}
...
func init() {
	...
	shared.Root.AddCommand(statusCmd)
}
